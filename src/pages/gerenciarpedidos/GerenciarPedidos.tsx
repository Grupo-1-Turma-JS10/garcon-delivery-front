    ...
    <div className="min-h-screen bg-gray-50 py-4 md:py-8 px-4 md:px-6">
      <div className="max-w-6xl mx-auto">
        <div className="mb-6 md:mb-8">
          <h1 className="text-2xl md:text-4xl font-bold text-gray-900 mb-1 md:mb-2">Gerenciar Pedidos</h1>
          <p className="text-sm md:text-base text-gray-600">Total de pedidos: {filteredOrders.length}</p>
        </div>
        ...
        {filteredOrders.length === 0 ? (
          <div className="bg-white rounded-lg shadow p-6 md:p-8 text-center">
            <p className="text-gray-500 text-base md:text-lg">Nenhum pedido encontrado neste status</p>
          </div>
        ) : (
              ...
              <div key={order.id} className="bg-white rounded-lg shadow-md hover:shadow-lg transition-shadow border-l-4 border-orange-500">
                <div className="p-4 md:p-6">
                  ...
                  <div className="mb-4 md:mb-6">
                    <h3 className="font-semibold text-base md:text-lg text-gray-900 mb-2 md:mb-3">Itens do Pedido</h3>
                      ...
                      {order.items.map((item, index) => (
                        <div key={index} className="flex flex-col sm:flex-row sm:justify-between sm:items-center text-sm md:text-base border-b border-gray-200 pb-2 last:border-0 gap-2 sm:gap-3">
                          ...
                          <div className="flex-1 min-w-0">
                            ...
                            {item.observations && <p className="text-gray-600 text-xs md:text-sm">Obs: {item.observations}</p>}
                          </div>
                          <div className="flex gap-2 sm:gap-3 flex-wrap sm:flex-nowrap text-right sm:text-left">
                            <p className="text-gray-700 text-sm"><strong>Qtd:</strong> {item.quantity}</p>
                            <p className="text-gray-700 text-sm whitespace-nowrap"><strong>Preço:</strong> {Intl.NumberFormat('pt-BR', {
                              style: 'currency',
                              currency: 'BRL'
                  ...
                  <div className="mb-4 md:mb-6 p-3 md:p-4 bg-orange-50 rounded-lg">
                    <p className="text-right text-lg md:text-2xl font-bold text-orange-600">
                  ...
                  {order.status !== 'CANCELED' && order.status !== 'FINISHED' &&
                    <div className="flex flex-col sm:flex-row gap-2 sm:gap-3">
                      <button
                        ...
                        className="flex-1 bg-orange-500 hover:bg-orange-600 text-white font-semibold py-2 px-4 rounded-lg transition-colors cursor-pointer text-sm md:text-base"
                      >
                        Editar
                      </button>
                      <button
                        onClick={() => handleDelete(order.id)}
                        className="flex-1 bg-red-500 hover:bg-red-600 text-white font-semibold py-2 px-4 rounded-lg transition-colors cursor-pointer text-sm md:text-base"
                      >
                        Cancelar
                      </button>
                    </div>
          ...
          >
            ...
            >
              <div className="p-4 md:p-6 border-b border-gray-200">
                <h2 className="text-xl md:text-2xl font-bold text-gray-900">Editar Pedido #{editedOrder.id}</h2>
              </div>
              ...
              >
                ...
                */}
                ...
                <div className="flex flex-col sm:flex-row gap-2 sm:gap-3 pt-4 border-t border-gray-200">
                  <button
                    type="submit"
                    className="flex-1 bg-orange-500 hover:bg-orange-600 text-white font-semibold py-2 px-4 rounded-lg transition-colors text-sm md:text-base"
                  >
                    Salvar Alterações
                  </button>
                  <button
