      ...
      <div className={`bg-white rounded-2xl shadow-xl p-6 sm:p-8 max-w-md w-full transition-all duration-300 ${showModal ? 'blur-sm pointer-events-none' : 'hover:shadow-2xl'}`}>
        ...
        <form onSubmit={handleSubmit} className="space-y-3 sm:space-y-4">
          <div>
            ...
            <input required type="text" placeholder="Digite seu nome completo" className="w-full px-3 sm:px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-orange-500 outline-none transition-all hover:border-orange-300" />
          </div>

          <div>
            <label className="block text-xs sm:text-sm font-medium mb-1 sm:mb-2 text-gray-700">CPF/CNPJ *</label>
            <input required type="text" placeholder="Digite seu CPF ou CNPJ" className="w-full px-3 sm:px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-orange-500 outline-none transition-all hover:border-orange-300" />
          </div>

          <div>
            <label className="block text-xs sm:text-sm font-medium mb-1 sm:mb-2 text-gray-700">Email *</label>
            <input required type="email" placeholder="seu@email.com" className="w-full px-3 sm:px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-orange-500 outline-none transition-all hover:border-orange-300" />
          </div>

          <div className="grid grid-cols-2 gap-3 sm:gap-4">
            <div>
              <label className="block text-xs sm:text-sm font-medium mb-1 sm:mb-2 text-gray-700">Senha *</label>
              <input required type="password" placeholder="Mínimo 6" className="w-full px-3 sm:px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-orange-500 outline-none transition-all hover:border-orange-300" />
            </div>
            <div>
              <label className="block text-xs sm:text-sm font-medium mb-1 sm:mb-2 text-gray-700">Confirmar *</label>
              <input required type="password" placeholder="Repita a senha" className="w-full px-3 sm:px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-orange-500 outline-none transition-all hover:border-orange-300" />
            </div>
          </div>
          ...
          <div>
            ...
            <div className="grid grid-cols-2 gap-3 sm:gap-4">
              <button type="button" onClick={() => setUserType('client')} className={`flex flex-col items-center justify-center p-3 sm:p-4 border-2 rounded-xl transition-all cursor-pointer ${userType === 'client' ? 'border-orange-500 bg-orange-50' : 'border-gray-300 hover:border-orange-300'}`}>
                <User className={`w-5 h-5 sm:w-6 sm:h-6 mb-1 sm:mb-2 ${userType === 'client' ? 'text-orange-600' : 'text-orange-500'}`} />
                <span className="font-medium text-xs sm:text-sm">Cliente</span>
              </button>
              <button type="button" onClick={() => setUserType('restaurant')} className={`flex flex-col items-center justify-center p-3 sm:p-4 border-2 rounded-xl transition-all cursor-pointer ${userType === 'restaurant' ? 'border-orange-500 bg-orange-50' : 'border-gray-300 hover:border-orange-300'}`}>
                <Store className={`w-5 h-5 sm:w-6 sm:h-6 mb-1 sm:mb-2 ${userType === 'restaurant' ? 'text-orange-600' : 'text-orange-500'}`} />
