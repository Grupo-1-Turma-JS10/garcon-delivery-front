...
import { ToastAlerta } from '../../utils/ToastAlerta'; // Adicionado import do Toast
...
export function Cadastro() {
  ...
  const cadastrarNovoUsuario = async () => {
    try {
      ...
      ToastAlerta('Erro ao realizar o cadastro. Tente novamente.', 'erro');
    } finally {
      setIsLoading(false);
    }
  };
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    // 1. Validação de Nome (Somente letras e espaços)
    const nomeRegex = /^[A-Za-zÀ-ÿ\s]+$/;
    if (!nomeRegex.test(formData.name)) {
      ToastAlerta('O nome deve conter apenas letras!', 'info');
      return;
    }

    // 2. Validação de CPF/CNPJ (Somente números)
    const documentoRegex = /^\d+$/;
    if (!documentoRegex.test(formData.document)) {
      ToastAlerta('O documento (CPF/CNPJ) deve conter apenas números!', 'info');
      return;
    }

    // 3. Validação de Senha (Confirmação)
    if (formData.password !== confirmPassword) {
      ToastAlerta('As senhas não coincidem!', 'info');
      return;
    }

    // 4. Validação de Senha (Tamanho mínimo)
    if (formData.password.length < 6) {
      ToastAlerta('A senha deve ter no mínimo 6 caracteres!', 'info');
      return;
    }

    // Se passar por todas as validações, ativa o loading e chama o cadastro
    setIsLoading(true);
